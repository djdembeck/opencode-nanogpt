...
describe('configureMcpEnvironment', () => {
  ...
  it('creates MCP section with correct structure', async () => {
    ...
    expect(mcpConfig.command).toEqual(['npx', '@nanogpt/mcp@latest', '--scope', 'user']);
    expect(mcpConfig.enabled).toBe(true);
  });

  it('uses {env:NANOGPT_MCP_API_KEY} syntax instead of actual API key', async () => {
    await writeFile(testFilePath, '{}');
    ...
    const mcpConfig = config.mcp.nanogpt as McpServerConfig;
    expect(mcpConfig.environment.NANOGPT_API_KEY).toBe('{env:NANOGPT_MCP_API_KEY}');
  ...
  it('does not modify other MCP servers', async () => {
    ...
    expect(config.mcp.nanogpt.command).toEqual(['npx', '@nanogpt/mcp@latest', '--scope', 'user']);
    expect(config.mcp.nanogpt.environment.NANOGPT_API_KEY).toBe('{env:NANOGPT_MCP_API_KEY}');
  ...
  it('sets command array correctly', async () => {
    ...
    expect(mcpConfig.command[0]).toBe('npx');
...
describe('updateNanogptProvider', () => {
  ...
  it('should update nanogpt models while preserving other providers', async () => {
    const initialConfig = {
      provider: {
        nanogpt: {
          ...
          models: { 'old-model': { id: 'old', name: 'Old Model', limit: { context: 4096, output: 1024 } } }
        },
        opencode: {
          npm: '@ai-sdk/opencode',
          name: 'OpenCode',
          models: { 'big-pickle': { name: 'Big Pickle', limit: { context: 128000, output: 4096 } } }
        },
        openai: {
          npm: '@ai-sdk/openai',
          name: 'OpenAI',
          models: { 'gpt-4': { name: 'GPT-4', limit: { context: 8192, output: 4096 } } }
        }
      }
    };
    ...
    expect(config.provider.nanogpt.options.baseURL).toBe('https://nano-gpt.com/api/v1');
...
describe('ensureNanogptProvider', () => {
  ...
  it('should create nanogpt provider with default structure', async () => {
    await writeTestFile(testFilePath, JSON.stringify({ provider: {} }, null, 2));
  ...
  it('should preserve existing nanogpt configuration', async () => {
    const existingProvider: NanogptProvider = {
      ...
      models: { 'custom-model': { id: 'custom', name: 'Custom', limit: { context: 1000, output: 500 } } }
    };
    await writeTestFile(testFilePath, JSON.stringify({ provider: { nanogpt: existingProvider } }, null, 2));
  ...
  it('should not modify other providers', async () => {
    const initialConfig = {
      provider: {
        opencode: { npm: '@opencode', name: 'OpenCode', options: { baseURL: 'https://opencode.ai' }, models: {} },
        openai: { npm: '@openai', name: 'OpenAI', options: { baseURL: 'https://api.openai.com' }, models: {} }
      }
    };
    await writeTestFile(testFilePath, JSON.stringify(initialConfig, null, 2));
...
describe('removeNanogptProvider', () => {
    ...
    const initialConfig = {
      provider: {
        nanogpt: {
          ...
          name: 'NanoGPT',
          options: { baseURL: 'https://nano-gpt.com/api/v1' },
          models: { 'model-1': { id: 'model-1', name: 'Model 1', limit: { context: 4096, output: 1024 } } }
        },
        opencode: { models: {} }
      }
    };
  ...
  it('should preserve other providers when removing nanogpt', async () => {
    const initialConfig = {
      provider: {
        nanogpt: { npm: '@ai-sdk/openai-compatible', name: 'NanoGPT', options: {}, models: {} },
        opencode: { npm: '@opencode', name: 'OpenCode', options: {}, models: { 'model-1': { name: 'Model 1', limit: { context: 1000, output: 500 } } } },
        openai: { npm: '@openai', name: 'OpenAI', options: {}, models: { 'gpt-4': { name: 'GPT-4', limit: { context: 8000, output: 4000 } } } }
      }
    };
    await writeTestFile(testFilePath, JSON.stringify(initialConfig, null, 2));
