...
5. **Manually Install MCP**:
   ```bash
   bun install -g @nanogpt/mcp@latest
   ```
...
3. **Test MCP Directly**:
   ...
   NANOGPT_API_KEY="your_key" bunx @nanogpt/mcp@latest
   ```
...
We recommend using cron for automatic updates instead of shell config files.

1. **Check if a cron job exists**:

   ```bash
   crontab -l | grep update-nanogpt-models
   ```
...
2. **Add a cron job**:
   ...
   # Add line for daily updates at 6 AM
